...
export function ExpiredDate() {
  ...
  async function getExpiredDateByKodeItem(item_kode: string, date_transaction: string, shift: string): Promise<{ outputDate: string, oldestDate: string }> {
    ...
    let getOutput = lists.value.filter((rec) => rec.item_kode == item_kode && rec.date_transaction == date_transaction && rec.shift == shift);

    if(!getOutput.length) getOutput = lists.value.filter((rec) => rec.item_kode == item_kode && rec.date_transaction == date_transaction);
